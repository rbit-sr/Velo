...
namespace Velo
{
    ...
    public enum EViolations
    {
        ...
        TOO_LONG, // run is longer than 2 minutes
...
54: { (1400, -10000, 100, 11420), (13950, 2630, 100, 410), (750, 1130, 100, 200) }, { }";
        ...
        {
            if (time >= 120f)
                Violations[(int)EViolations.TOO_LONG] = "time is longer than 2 minutes (" + time + "s)";
            if (MapId != -1)
            {
        ...
        public void Update()
        {
            ...
            if (Velo.LastUsedItemTime <= 5f)
                Violations[(int)EViolations.ITEM_USED] = "item " + Velo.LastUsedItemId + " used";
            if (Velo.LastUsedDrillTime <= 5f)
